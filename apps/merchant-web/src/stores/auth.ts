...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function exchangeAdminHandoff(token: string): Promise<void> {
    loading.value = true
    errorMessage.value = ''
    try {
      const result = await authService.exchangeAdminHandoff(token)
      const localMarker = isRealApiMode ? 'cookie-session' : result.sessionId
      sessionId.value = localMarker
      sessionStorage.setItem(isRealApiMode ? AUTH_MARKER_KEY : MOCK_SESSION_KEY, localMarker)
    } catch (error) {
      errorMessage.value = error instanceof Error ? error.message : '安全跳转失败，请返回管理后台重试'
      throw error
    } finally {
      loading.value = false
    }
