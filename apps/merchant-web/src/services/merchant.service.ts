...
export function deleteQuestionsBatch(input: QuestionBatchDeleteRequest): Promise<QuestionBatchDeleteResponse> {
  return apiRequest<QuestionBatchDeleteResponse>('/merchant/questions/batch-delete', {
    method: 'POST',
    body: JSON.stringify(input),
  })
}
...
export function listArticleGroups(): Promise<ArticleGroup[]> {
  return apiRequest<ArticleGroup[]>('/merchant/article-groups')
}

export function updateArticleGroup(groupId: string, input: ArticleGroupInput): Promise<ArticleGroup> {
  return apiRequest<ArticleGroup>(`/merchant/article-groups/${encodeURIComponent(groupId)}`, { method: 'PATCH', body: JSON.stringify(input) })
}

export function listArticles(groupId?: string): Promise<MerchantArticle[]> {
  const query = groupId ? `?groupId=${encodeURIComponent(groupId)}` : ''
  return apiRequest<MerchantArticle[]>(`/merchant/articles${query}`)
}
...
export function retryArticleAiTask(taskId: string, referenceId?: string): Promise<AiGenerationTask> {
  return apiRequest<AiGenerationTask>(`/merchant/ai-tasks/${encodeURIComponent(taskId)}/retry`, {
    method: 'POST',
    headers: { 'idempotency-key': crypto.randomUUID() },
    body: JSON.stringify({ referenceId: referenceId ?? null }),
  })
}
...
export function createWebsiteAiProfile(input: WebsiteAiProfileInput): Promise<WebsiteProfileDraft> {
  return apiRequest<WebsiteProfileDraft>('/merchant/website/ai-profile', { method: 'POST', body: JSON.stringify(input) })
}
...
export function createPublishTasks(input: CreatePublishTasksRequest, idempotencyKey = crypto.randomUUID()): Promise<CreatePublishBatchResponse> {
