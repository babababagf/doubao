...
function buildMockDashboardTrend(): DashboardResponse['dailyTrend'] {
  ...
  return Array.from({ length: 30 }, (_, index) => {
    ...
    return {
      ...
      keywordCount: index % 9 === 0 ? 2 : index % 4 === 0 ? 1 : 0,
      expandedQuestionCount: 2 + ((index * 3) % 8),
      aiWritingCount: index % 3 === 0 ? 3 : index % 2 === 0 ? 2 : 1,
      publishedCount: index % 5 === 0 ? 5 : 2 + (index % 3),
      doubaoIncludedCount: index % 4 === 0 ? 3 : index % 3 === 0 ? 2 : 1,
      phoneExposureCount: 5 + ((index * 7) % 16),
      phoneClickCount: 1 + ((index * 2) % 6),
    }
  })
}
...
export const mockDashboard: DashboardResponse = {
  ...
  dailyTrend: buildMockDashboardTrend(),
  resources: {
...
let mockProfile: MerchantProfile = {
  ...
  nearbyLandmark: '示例中心东门旁，距地铁站约 300 米',
  transportGuide: '地铁 1 号线示例站 A 口出站，步行约 5 分钟',
  parkingGuide: '园区地下停车场可临时停车，请以现场指引为准',
  phoneLabel: '业务咨询',
  phone: '400-888-2026',
  phoneContacts: [{ label: '业务咨询', phone: '400-888-2026' }],
  wechatLabel: '添加企业微信',
  wechat: 'demo-growth',
...
let mockQuestions: MerchantQuestion[] = Array.from({ length: 128 }, (_, index) => {
  ...
  return {
    ...
    text: `${keyword?.name ?? '企业内容获客'}${pattern}`,
...
let mockGalleryImages: GalleryImage[] = [
  { id: 'image-1', galleryId: 'gallery-1', fileName: 'brand-logo.png', mimeType: 'image/png', sizeBytes: 183204, formattedSize: '178.9 KB', usageCount: 0, createdAt: '2026-08-05T15:20:00+08:00' },
  { id: 'image-2', galleryId: 'gallery-1', fileName: 'office-space.jpg', mimeType: 'image/jpeg', sizeBytes: 2167421, formattedSize: '2.1 MB', usageCount: 0, createdAt: '2026-08-05T15:18:00+08:00' },
  { id: 'image-3', galleryId: 'gallery-2', fileName: 'service-workflow.jpg', mimeType: 'image/jpeg', sizeBytes: 1860402, formattedSize: '1.8 MB', usageCount: 0, createdAt: '2026-08-04T14:10:00+08:00' },
  { id: 'image-4', galleryId: 'gallery-2', fileName: 'product-demo.jpg', mimeType: 'image/jpeg', sizeBytes: 2443341, formattedSize: '2.3 MB', usageCount: 0, createdAt: '2026-08-04T14:08:00+08:00' },
]
...
内容参数：全文 1000-1200 字，设置 3-4 个分级标题，单段不超过 150 字，融入 2-3 个模糊化处理的数据支撑点。
图片规则：正文合理位置的配图由系统根据图库快照插入，模型不输出图片链接、Markdown 图片语法或 {图片链接} 占位符。
...
# 输出规格与注意事项
按系统要求输出结构化 HTML 格式完整文章，字数严格控制在 1000-1200 字，结尾不设置 FAQ 板块；不输出 Markdown、脚本、样式、图片链接或额外说明。
...
let mockArticles: MerchantArticle[] = [
  {
    id: 'article-1',
    articleGroupId: 'article-group-1',
    title: '企业内容获客如何从真实资料开始搭建？',
    content: '企业内容获客首先要建立统一的企业事实源，再围绕真实业务问题沉淀可持续更新的内容。本文为待完善的 AI 草稿，可在文章列表中继续编辑。',
    source: 'ai_mock',
    status: 'draft',
    keywordId: 'keyword-1',
...
let mockArticleGroups: ArticleGroup[] = [
  { id: 'article-group-1', name: '2026-08 企业内容获客', requestedCount: 1, completedCount: 1, status: 'completed', createdAt: '2026-08-06T10:18:00+08:00', updatedAt: '2026-08-06T10:18:00+08:00', isUngrouped: false },
]
...
const mockMediaAccounts: MediaAccount[] = [
  { id: 'media-tt-1', platform: 'toutiao', status: 'connected', maskedName: '内容运营主号', avatarUrl: null, localReferenceId: 'local-ref-tt-8f2c', lastVerifiedAt: '2026-08-06T09:20:00+08:00', lastHeartbeatAt: '2026-08-06T10:00:00+08:00', failureReason: null, backupAvailable: true, backupCapturedAt: '2026-08-06T09:21:00+08:00' },
  { id: 'media-tt-2', platform: 'toutiao', status: 'connected', maskedName: '内容运营分号', avatarUrl: null, localReferenceId: 'local-ref-tt-a221', lastVerifiedAt: '2026-08-06T09:18:00+08:00', lastHeartbeatAt: '2026-08-06T10:00:00+08:00', failureReason: null, backupAvailable: true, backupCapturedAt: '2026-08-06T09:19:00+08:00' },
  { id: 'media-dy-1', platform: 'douyin', status: 'connected', maskedName: '品牌增长主号', avatarUrl: null, localReferenceId: 'local-ref-dy-57a1', lastVerifiedAt: '2026-08-05T14:10:00+08:00', lastHeartbeatAt: '2026-08-06T08:20:00+08:00', failureReason: null, backupAvailable: true, backupCapturedAt: '2026-08-05T14:11:00+08:00' },
  { id: 'media-dy-2', platform: 'douyin', status: 'verification_required', maskedName: '品牌增长备用号', avatarUrl: null, localReferenceId: 'local-ref-dy-712d', lastVerifiedAt: '2026-08-05T14:10:00+08:00', lastHeartbeatAt: '2026-08-06T08:20:00+08:00', failureReason: '本地助手提示需要重新完成平台验证', backupAvailable: false, backupCapturedAt: null },
  { id: 'media-smzdm-1', platform: 'smzdm', status: 'connected', maskedName: '品牌内容值友号', avatarUrl: null, localReferenceId: 'local-ref-smzdm-281a', lastVerifiedAt: '2026-08-06T09:30:00+08:00', lastHeartbeatAt: '2026-08-06T10:00:00+08:00', failureReason: null, backupAvailable: true, backupCapturedAt: '2026-08-06T09:31:00+08:00' },
]
let mockPublishTasks: PublishTask[] = [
  { id: 'publish-1', batchId: 'batch-history-1', taskName: '品牌内容首发', articleGroupId: null, articleGroupName: '全部文章库', articleId: 'article-1', articleVersion: 1, articleTitle: '企业内容获客如何从真实资料开始搭建？', platform: 'toutiao', mediaAccountId: 'media-tt-1', mediaAccountName: '内容运营主号', status: 'succeeded', progress: 100, currentStep: 'completed', scheduledAt: '2026-08-06T09:00:00+08:00', startedAt: '2026-08-06T09:01:00+08:00', createdAt: '2026-08-06T09:00:00+08:00', updatedAt: '2026-08-06T09:05:00+08:00', completedAt: '2026-08-06T09:05:00+08:00', publishedAt: '2026-08-06T09:05:00+08:00', resultUrl: 'https://www.toutiao.com/article/mock-1', failureReason: null, attentionReason: null, canPause: false, canResume: false, attemptCount: 1, logs: [] },
  { id: 'publish-2', batchId: 'batch-history-2', taskName: '官网内容同步', articleGroupId: null, articleGroupName: '全部文章库', articleId: 'article-2', articleVersion: 1, articleTitle: '官网内容建设中常见的三个事实一致性问题', platform: 'douyin', mediaAccountId: 'media-dy-1', mediaAccountName: '品牌增长主号', status: 'queued', progress: 0, currentStep: 'waiting', scheduledAt: '2026-08-06T10:02:00+08:00', startedAt: null, createdAt: '2026-08-06T10:02:00+08:00', updatedAt: '2026-08-06T10:02:00+08:00', completedAt: null, publishedAt: null, resultUrl: null, failureReason: null, attentionReason: null, canPause: true, canResume: false, attemptCount: 0, logs: [] },
]
...
export function syncMockDashboardStats(): void {
  ...
  mockDashboard.overview.keywordCount = keywords.length
  mockDashboard.overview.expandedQuestionCount = mockDashboard.effects.questionTotal
  mockDashboard.overview.aiWritingCount = mockArticles.filter((article) => article.source !== 'manual').length
  mockDashboard.overview.publishedCount = mockDashboard.resources.publishCount
  mockDashboard.overview.doubaoIncludedCount = mockDashboard.effects.doubaoIncludedCount
  mockDashboard.overview.phoneExposureCount = mockDashboard.effects.phoneExposureCount
  mockDashboard.overview.phoneClickCount = mockDashboard.effects.phoneClickCount
}
...
export function deleteMockQuestions(questionIds: string[]): number | null {
  const ids = [...new Set(questionIds)]
  const liveIds = new Set(mockQuestions.filter((item) => ids.includes(item.id) && !deletedQuestionIds.has(item.id)).map((item) => item.id))
  if (liveIds.size !== ids.length) return null
  for (const id of ids) deletedQuestionIds.add(id)
  syncMockDashboardStats()
  return ids.length
}
...
export function listMockGalleryImages(galleryId: string): GalleryImage[] | undefined {
  ...
  return mockGalleryImages
    ...
    .map((image) => ({
      ...image,
      usageCount: mockArticles.filter((article) => article.galleryId === galleryId && (article.coverImageId === image.id || article.galleryImageIds.includes(image.id))).length,
    }))
    .map(deepClone)
}
...
export function addMockGalleryImage(
  ...
  const { purpose: _purpose, ...imageInput } = input
  void _purpose
  const image: GalleryImage = {
    id: `image-${Date.now()}`,
    galleryId,
    ...deepClone(imageInput),
...
export function updateMockWritingInstruction(
  ...
  Object.assign(instruction, deepClone(input), instruction.isSystem ? { name: '系统默认内容指令' } : {}, { updatedAt: new Date().toISOString() })
  return deepClone(instruction)
}
...
export function listMockArticleGroups(): ArticleGroup[] {
  const groups = mockArticleGroups.map((group) => {
    const completedCount = mockArticles.filter((article) => article.articleGroupId === group.id).length
    return { ...group, completedCount, status: completedCount >= group.requestedCount ? 'completed' as const : completedCount > 0 ? 'partial' as const : group.status }
  })
  const ungroupedCount = mockArticles.filter((article) => !article.articleGroupId).length
  if (ungroupedCount) groups.push({ id: 'ungrouped', name: '未分组文章', requestedCount: ungroupedCount, completedCount: ungroupedCount, status: 'completed', createdAt: new Date(0).toISOString(), updatedAt: new Date(0).toISOString(), isUngrouped: true })
  return groups.map(deepClone)
}

export function updateMockArticleGroup(groupId: string, name: string): ArticleGroup | undefined {
  const group = mockArticleGroups.find((item) => item.id === groupId)
...
export function listMockArticles(groupId?: string): MerchantArticle[] {
  const articles = groupId ? mockArticles.filter((article) => groupId === 'ungrouped' ? !article.articleGroupId : article.articleGroupId === groupId) : mockArticles
  return articles.slice().sort((left, right) => right.updatedAt.localeCompare(left.updatedAt)).map(deepClone)
}
...
export function createMockArticles(input: MockArticleCreateRequest): MockArticleCreateResponse | undefined {
  ...
  const referenceQuestions = activeQuestions(input.keywordId)
  if (!referenceQuestions.length) return undefined

  const now = new Date().toISOString()
  const groupId = `article-group-${Date.now()}`
  mockArticleGroups = [{ id: groupId, name: input.groupName, requestedCount: input.count, completedCount: input.count, status: 'completed', createdAt: now, updatedAt: now, isUngrouped: false }, ...mockArticleGroups]
  ...
  const articles = Array.from({ length: input.count }, (_, index) => {
    ...
    const question = referenceQuestions[Math.floor(Math.random() * referenceQuestions.length)]!
    referencedQuestionIds.add(question.id)
    const article: MerchantArticle = {
      id: `article-${Date.now()}-${index + 1}`,
      articleGroupId: groupId,
      title: input.customTitles?.[index] || question.text,
      content: `本文按“${directionLabels[direction]}”方向，围绕“${keyword.name}”和问题“${question.text}”整理内容。${input.knowledgeLibraryIds.length ? '当前使用事实增强模式，写作会结合已选企业信息库。' : '当前使用基础写作模式，可结合公司名、关键词和问题词生成内容。'}这是第 ${sequence} 篇待完善的 AI 草稿，可在文章列表中继续编辑与补充事实。`,
      source: 'ai_mock',
      status: 'draft',
  ...
  referenceQuestions.filter((question) => referencedQuestionIds.has(question.id)).forEach((question) => { question.articleCreated = true })
  syncMockDashboardStats()
...
export function updateMockWebsite(input: MerchantWebsiteInput): MerchantWebsite {
  mockWebsite = {
    ...
    showcaseImageIds: input.showcaseImageIds ?? [],
    status: 'not_generated',
    previewUrl: null,
...
export function createMockWebsiteProfileDraft(libraryId: string): WebsiteProfileDraft | undefined {
  const library = mockKnowledgeLibraries.find((item) => item.id === libraryId)
  if (!library) return undefined
  const lines = (value: string): string[] => value.split(/[\n；;]/).map((item) => item.trim()).filter(Boolean)
  return {
    companyName: library.companyName,
    aliases: [library.brandAlias].filter(Boolean),
    industry: '',
    coreBusiness: lines(library.productServices)[0] ?? '',
    serviceAreas: [],
    introduction: library.brandStory || library.otherInfo,
    advantages: lines(library.productFeatures),
    products: lines(library.productServices),
    address: '',
    nearbyLandmark: '',
    transportGuide: '',
    parkingGuide: '',
    businessHours: '',
    credentials: lines(library.trustProof),
    cases: lines(library.customerCases),
    proofMaterials: [],
  }
}
...
export function requestMockMediaConnect(platform: MediaAccount['platform']): MediaAccount {
  const account: MediaAccount = { id: `media-${platform}-${Date.now()}`, platform, status: 'connection_requested', maskedName: null, avatarUrl: null, localReferenceId: null, lastVerifiedAt: null, lastHeartbeatAt: null, failureReason: null, backupAvailable: false, backupCapturedAt: null }
...
export function createMockPublishTasks(input: CreatePublishTasksRequest): CreatePublishBatchResponse | undefined {
  const articleGroup = mockArticleGroups.find((group) => group.id === input.articleGroupId && !group.isUngrouped)
  if (!articleGroup) return undefined
  const articles = mockArticles
    .filter((article) => article.articleGroupId === input.articleGroupId && article.status === 'publishable')
    .sort((left, right) => right.updatedAt.localeCompare(left.updatedAt) || left.id.localeCompare(right.id))
  const requiredArticleCount = input.deduplicationMode === 'all_platforms'
    ? input.platforms.reduce((total, platform) => total + input.publishCounts[platform], 0)
    : Math.max(...input.platforms.map((platform) => input.publishCounts[platform]))
  if (articles.length < requiredArticleCount) return undefined
  ...
  const assignments = input.deduplicationMode === 'all_platforms'
    ? input.platforms.flatMap((platform) => {
        const selected = articles.slice(cursor, cursor + input.publishCounts[platform])
        cursor += selected.length
        return selected.map((article) => ({ article, platform }))
      })
    : input.platforms.flatMap((platform) => articles.slice(0, input.publishCounts[platform]).map((article) => ({ article, platform })))
  const articleGroupName = articleGroup.name
  const tasks: PublishTask[] = assignments.map(({ article, platform }, index) => {
    ...
    return { id:`publish-${Date.now()}-${index}`, batchId, taskName: input.taskName, articleGroupId: input.articleGroupId, articleGroupName, articleId:article.id, articleVersion:article.currentVersion, articleTitle:article.title, platform, mediaAccountId: account.id, mediaAccountName: account.maskedName, status:'queued', progress:0, currentStep:'waiting', scheduledAt: now, startedAt:null, createdAt:now, updatedAt:now, completedAt:null, publishedAt:null, resultUrl:null, failureReason:null, attentionReason:null, canPause:true, canResume:false, attemptCount:0, logs:[] }
  })
  mockPublishTasks=[...tasks,...mockPublishTasks]
  return deepClone({ batchId, taskName: input.taskName, articleGroupId: input.articleGroupId, articleGroupName, publishCounts: input.publishCounts, createdTaskCount: 1, createdExecutionCount: tasks.length, skippedDuplicateCount: 0, estimatedTaskCount: 1, estimatedExecutionCount: tasks.length, tasks })
}
