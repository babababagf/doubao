...
export const MOCK_SESSION_ID = 'mock-session-demo001'
...
export const handlers = [
  http.post('/api/auth/login', async ({ request }) => {
    ...
    if (body.username !== 'demo001' || body.password !== 'demo123') {
      return HttpResponse.json(
        {
  ...
  http.put('/api/merchant/writing-instructions/:instructionId', async ({ request, params }) => {
    ...
    if (instruction === 'system') return apiError(403, 'SYSTEM_INSTRUCTION_READ_ONLY', '系统默认指令不可修改')
  ...
  http.get('/api/merchant/articles', async ({ request }) => {
    await delay(200)
    return hasValidSession(request) ? HttpResponse.json(listMockArticles()) : unauthorized()
  }),
  ...
  http.post('/api/merchant/articles/create-mock', async ({ request }) => {
    ...
    if (body.count > keyword.uncreatedCount) {
      return apiError(422, 'MOCK_ARTICLE_QUESTION_LIMIT', `当前关键词仅剩 ${keyword.uncreatedCount} 个可用问题词，不能创建 ${body.count} 篇文章`)
    }
  ...
  http.post('/api/merchant/media-accounts/:platform/connect-mock', async ({ request, params }) => {
    ...
    return ['toutiao','douyin'].includes(platform) ? HttpResponse.json(requestMockMediaConnect(platform as 'toutiao' | 'douyin')) : apiError(422,'MEDIA_PLATFORM_INVALID','仅支持今日头条或抖音')
  }),
  ...
  http.post('/api/merchant/publish-tasks', async ({ request }) => {
    ...
    if (!Array.isArray(body.articleIds) || !body.articleIds.length || !Array.isArray(body.platforms) || !body.platforms.length || !Array.isArray(body.mediaAccountIds) || !body.mediaAccountIds.length) return apiError(422,'PUBLISH_TASK_INPUT_INVALID','请选择文章、发布平台和已连接账号')
    const batch = createMockPublishTasks(body)
    return batch ? HttpResponse.json(batch,{status:201}) : apiError(409,'PUBLISH_TASK_CREATE_FAILED','文章或发布账号不可用')
  }),
]
